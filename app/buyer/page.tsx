...
function ProductCard({ p, view, cart, onCart, wishlist, onWishlist }: {
    ...
    if (view === 'list') return (
        <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
            ...
            <div style={{ width:68, height:68, borderRadius:4, flexShrink:0, background:`${p.sellerAccent}15`, border:`1px solid ${p.sellerAccent}28`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:28 }}>{p.icon}</div>
            <div style={{ flex:1, minWidth:0 }}>
                <div style={{ display:'flex', alignItems:'center', gap:7, marginBottom:4, flexWrap:'wrap' }}>
                    ...
                    {p.isListed && <span style={{ fontSize:9, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', padding:'2px 7px', borderRadius:2, background:'rgba(0,229,255,0.12)', border:'1px solid rgba(0,229,255,0.3)', color:'#00e5ff' }}>New Listing</span>}
        ...
        <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
            ...
            <div style={{ position:'relative', height:150, background:`linear-gradient(135deg,${p.sellerAccent}0a,rgba(0,229,255,0.03))`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:48, borderBottom:`1px solid ${C.border}` }}>
                ...
                <div style={{ position:'absolute', top:10, left:10, display:'flex', flexDirection:'column', gap:4 }}>
                    {p.isListed && <span style={{ fontSize:9, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', padding:'3px 7px', borderRadius:2, background:'rgba(0,229,255,0.18)', border:'1px solid rgba(0,229,255,0.4)', color:'#00e5ff' }}>New</span>}
                ...
                {disc > 0 && <div style={{ position:'absolute', top:10, right:10, fontSize:10, fontWeight:700, padding:'3px 7px', borderRadius:2, background:'rgba(74,222,128,0.12)', border:'1px solid rgba(74,222,128,0.28)', color:'#4ade80' }}>{disc}% OFF</div>}
            ...
            <div style={{ padding:'14px 16px', flex:1, display:'flex', flexDirection:'column' }}>
                ...
                <div style={{ display:'flex', alignItems:'baseline', gap:8, marginBottom:12 }}>
                    ...
                    {disc > 0 && <span style={{ fontSize:12, color:C.dim, textDecoration:'line-through' }}>₹{p.originalPrice.toLocaleString()}</span>}
                </div>
                <button onClick={() => onCart(p.id)} disabled={!p.inStock}
...
function CartDrawer({ cart, allProducts, onClose, onQty, onRemove }: {
    cart: CartItem[]; allProducts: Product[]; onClose: () => void;
                ...
                <div style={{ flex:1, overflowY:'auto', padding:'10px 20px' }}>
                    {items.length === 0
                        ? <div style={{ textAlign:'center', paddingTop:60 }}>
                            <ShoppingCart style={{ width:30, height:30, color:C.dim, margin:'0 auto 10px' }} />
                            <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:700, fontSize:15, textTransform:'uppercase', color:C.dim }}>Cart is empty</div>
                        </div>
                        : items.map(({ p, qty }) => (
...
export default function BuyerPage() {
    ...
    useEffect(() => {
        const load = () => {
            try {
                ...
                const shaped: Product[] = parsed.map((p) => ({
                    id:             String(p.id ?? ''),
                    name:           String(p.name ?? 'Unnamed Product'),
                    sellerName:     String(p.sellerName ?? 'Unknown Seller'),
                    sellerLocation: String(p.sellerLocation ?? 'India'),
                    sellerAccent:   String(p.sellerAccent ?? '#00e5ff'),
                    sellerCategory: String(p.sellerCategory ?? 'handicraft'),
                    sellerRating:   Number(p.sellerRating ?? 4.5),
                    category:       String(p.category ?? 'handicraft'),
                    price:          Number(p.price ?? 0),
                    originalPrice:  Number(p.originalPrice ?? p.price ?? 0),
                    desc:           String(p.desc ?? ''),
                    inStock:        Boolean(p.inStock ?? true),
                    badge:          String(p.badge ?? 'New'),
                    tag:            p.tag ? String(p.tag) : null,
                    tagColor:       p.tagColor ? String(p.tagColor) : null,
                    icon:           String(p.icon ?? '📦'),
                    delivery:       String(p.delivery ?? '5-7 days'),
                    isListed:       true,
                }));
                setListedProducts(shaped);
            } catch {}
        };
        load();
    ...
    const initials  = user
        ? (`${user.firstName?.charAt(0) ?? ''}${user.lastName?.charAt(0) ?? ''}`).toUpperCase() || 'GU'
        : 'GU';
    ...
    const filtered = useMemo((): Product[] => {
        ...
        if (sortBy === 'newest')    list = [...list].sort((a, b) => (b.isListed ? 1 : 0) - (a.isListed ? 1 : 0));
        ...
        return list;
    }, [ALL_PRODUCTS, query, activeCategory, activeState, sortBy]);
            ...
            <div className="bp" style={{ minHeight:'100vh', background:C.bg }}>
                ...
                <div style={{ maxWidth:1200, margin:'0 auto', padding:'28px 24px 60px' }}>
                        ...
                        <div style={{ position:'relative', maxWidth:520 }}>
                            ...
                            <div style={{ display:'flex', gap:20, flexWrap:'wrap' }}>
                                {[
                                    ...
                                    { v:`${SELLERS.length}${listedProducts.length > 0 ? '+' : ''}`, l:'Sellers' },
                    ...
                    <div className="fu2" style={{ display:'flex', marginBottom:20, background:'rgba(17,31,42,0.7)', border:`1px solid rgba(0,229,255,0.08)`, borderRadius:5, overflow:'hidden' }}>
                        {[
                            { icon:<Shield style={{width:12,height:12}}/>,    l:'Verified Sellers' },
                            { icon:<Truck style={{width:12,height:12}}/>,     l:'Pan-India Delivery' },
                            { icon:<Package style={{width:12,height:12}}/>,   l:'Secure Packaging' },
                            { icon:<BadgeCheck style={{width:12,height:12}}/>,l:'Authentic Handmade' },
                        ].map((t, i) => (
                    ...
                    <div className="fu3" style={{ display:'flex', gap:6, flexWrap:'wrap', marginBottom:16 }}>
                        {CATEGORIES.map(c => (
                            <button key={c.id} className={`cat-pill${activeCategory === c.id ? ' active' : ''}`} onClick={() => setActiveCategory(c.id)}>{c.label}</button>
                        ))}
                    </div>
                    ...
                    <div className="fu3" style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16, flexWrap:'wrap', gap:8 }}>
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                            ...
                            <div style={{ position:'relative' }}>
                                ...
                                {showSort && (
                                    <div style={{ position:'absolute', top:'100%', right:0, marginTop:4, background:'#0d1b24', border:`1px solid ${C.accentBd}`, borderRadius:5, overflow:'hidden', zIndex:20, minWidth:160, boxShadow:'0 12px 32px rgba(0,0,0,0.5)' }}>
                                        {SORT_OPTIONS.map(o => (
                    ...
                    <div className="fu5" style={{
                        ...
                        {filtered.map(p => (
                            <ProductCard key={p.id} p={p} view={view} cart={cart} onCart={toggleCart} wishlist={wishlist} onWishlist={toggleWish} />
                        ))}
                        {filtered.length === 0 && (
                ...
                {cartOpen && <CartDrawer cart={cart} allProducts={ALL_PRODUCTS} onClose={() => setCartOpen(false)} onQty={updateQty} onRemove={removeCart} />}
            </div>
        </>
    );
}