...
import {
    ...
    Grid3X3, List, ArrowRight, ShoppingCart,
    Heart, ChevronDown, X, SlidersHorizontal, Flame,
    BadgeCheck, Package, Truck, Shield,
...
const C = {
    bg:       '#0d1b24',
    bgCard:   'rgba(17,31,42,0.85)',
    bgHover:  'rgba(0,229,255,0.03)',
    accent:   '#00e5ff',
    accentDim:'rgba(0,229,255,0.12)',
    accentBd: 'rgba(0,229,255,0.2)',
    text:     '#e2e8f0',
    muted:    '#64748b',
    dim:      '#334155',
    border:   'rgba(255,255,255,0.06)',
    borderHov:'rgba(0,229,255,0.25)',
};
...
const PRODUCTS = [
    { id:1,  name:'Blue Pottery Vase',       seller:'Meena Crafts',      location:'Jaipur, Rajasthan',    category:'handicraft', price:1299, originalPrice:1799,  rating:4.9, reviews:312, tag:'Best Seller', tagColor:'#f97316', accent:'#f97316', badge:'Handmade',     desc:'Authentic Jaipur blue pottery vase with intricate floral motifs, hand-painted by master craftsmen.', delivery:'3-5 days', inStock:true,  icon:'🏺' },
    { id:2,  name:'Banarasi Silk Saree',      seller:'Silk Route Studio', location:'Varanasi, UP',         category:'textile',    price:8499, originalPrice:12000, rating:4.8, reviews:187, tag:'Trending',    tagColor:'#a78bfa', accent:'#a78bfa', badge:'Handwoven',    desc:'Pure Banarasi silk saree with zari brocade work. Traditional design from 5 generations.',           delivery:'5-7 days', inStock:true,  icon:'🧵' },
    { id:3,  name:'Sambalpuri Ikat Stole',    seller:'Tribal Threads',    location:'Bhubaneswar, Odisha',  category:'textile',    price:2199, originalPrice:2800,  rating:4.7, reviews:98,  tag:'New',         tagColor:'#00e5ff', accent:'#00e5ff', badge:'Tribal Art',   desc:'Hand-woven Sambalpuri ikat stole using traditional double ikat technique. Rich cotton fabric.',      delivery:'4-6 days', inStock:true,  icon:'🧣' },
    { id:4,  name:'Dhokra Tribal Necklace',   seller:'Dhokra Works',      location:'Bastar, Chhattisgarh', category:'jewellery',  price:3499, originalPrice:4200,  rating:4.9, reviews:74,  tag:'Rare',        tagColor:'#eab308', accent:'#eab308', badge:'Lost-wax Cast', desc:'Authentic Dhokra brass necklace using 4000-year-old lost-wax casting technique.',                   delivery:'6-8 days', inStock:true,  icon:'📿' },
    { id:5,  name:'Ceramic Mug Set (4pc)',    seller:'Mitti & More',      location:'Khurja, UP',           category:'pottery',    price:1599, originalPrice:2000,  rating:4.6, reviews:143, tag:null,          tagColor:null,      accent:'#22d3ee', badge:'Glazed',       desc:'Set of 4 hand-thrown glazed ceramic mugs. Microwave and dishwasher safe.',                          delivery:'3-5 days', inStock:true,  icon:'☕' },
    { id:6,  name:'Kerala Pepper 500g',       seller:'Spice Trail Co.',   location:'Kochi, Kerala',        category:'food',       price:499,  originalPrice:699,   rating:4.8, reviews:421, tag:'Top Rated',   tagColor:'#22c55e', accent:'#22c55e', badge:'Organic',      desc:'Single-origin Malabar black pepper. Cold-dried to preserve essential oils and full aroma.',          delivery:'2-3 days', inStock:true,  icon:'🌶️' },
    { id:7,  name:'Handloom Cotton Dupatta',  seller:'Tribal Threads',    location:'Bhubaneswar, Odisha',  category:'textile',    price:899,  originalPrice:1200,  rating:4.5, reviews:56,  tag:null,          tagColor:null,      accent:'#00e5ff', badge:'Handloom',     desc:'Lightweight handloom cotton dupatta with block print border. Natural dyes, breathable weave.',       delivery:'4-6 days', inStock:false, icon:'🧶' },
    { id:8,  name:'Block Print Kurta Fabric', seller:'Meena Crafts',      location:'Jaipur, Rajasthan',    category:'handicraft', price:1850, originalPrice:2400,  rating:4.7, reviews:89,  tag:'Limited',     tagColor:'#f97316', accent:'#f97316', badge:'Hand Block',   desc:'2.5m Sanganeri block print fabric. Hand-stamped with natural indigo dye on pure cotton.',            delivery:'3-5 days', inStock:true,  icon:'🖼️' },
    { id:9,  name:'Madhubani Painting (A3)',  seller:'Bihar Kala Kendra', location:'Madhubani, Bihar',     category:'handicraft', price:2200, originalPrice:3000,  rating:4.8, reviews:52,  tag:'New',         tagColor:'#00e5ff', accent:'#00e5ff', badge:'Folk Art',      desc:'Hand-painted Madhubani art on cotton cloth. Traditional Bihar folk motifs by certified artists.',    delivery:'5-7 days', inStock:true,  icon:'🎨' },
    { id:10, name:'Sikki Grass Basket',       seller:'Bihar Kala Kendra', location:'Madhubani, Bihar',     category:'handicraft', price:750,  originalPrice:1000,  rating:4.6, reviews:38,  tag:null,          tagColor:null,      accent:'#22c55e', badge:'Handcraft',    desc:'Traditional Sikki grass basket with geometric patterns. Used for storage and gifting.',              delivery:'4-6 days', inStock:true,  icon:'🧺' },
];
...
function ProductCard({ p, view, cart, onCart, wishlist, onWishlist }: {
    p:typeof PRODUCTS[0]; view:'grid'|'list';
    cart:CartItem[]; onCart:(id:number)=>void;
    wishlist:number[]; onWishlist:(id:number)=>void;
}) {
    ...
    if (view === 'list') return (
        <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
            ...
            <div style={{ position:'absolute', left:0, top:0, bottom:0, width:2, background:`linear-gradient(180deg,${p.accent},transparent)`, transform:hov?'scaleY(1)':'scaleY(0)', transformOrigin:'top', transition:'transform 0.22s ease' }} />
            <div style={{ width:68, height:68, borderRadius:4, flexShrink:0, background:`${p.accent}15`, border:`1px solid ${p.accent}28`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:28 }}>{p.icon}</div>
            <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:4, flexWrap:'wrap' }}>
                    <span style={{ fontSize:11, color:C.muted, display:'flex', alignItems:'center', gap:3 }}><BadgeCheck style={{ width:10,height:10,color:C.accent }} />{p.seller} · {p.location}</span>
                    <span style={{ display:'flex', alignItems:'center', gap:3, fontSize:11 }}><Star style={{ width:10,height:10,fill:'#eab308',color:'#eab308' }} /><span style={{ color:C.text,fontWeight:600 }}>{p.rating}</span><span style={{ color:C.dim }}>({p.reviews})</span></span>
    ...
    return (
        <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
            ...
            <div style={{ position:'absolute', top:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${p.accent},transparent)`, transform:hov?'scaleX(1)':'scaleX(0)', transformOrigin:'left', transition:'transform 0.28s ease', zIndex:2 }} />
            <div style={{ position:'relative', height:150, background:`linear-gradient(135deg,${p.accent}0a,rgba(0,229,255,0.03))`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:48, borderBottom:`1px solid ${C.border}` }}>
                {p.icon}
            ...
            <div style={{ padding:'14px 16px', flex:1, display:'flex', flexDirection:'column' }}>
                <span style={{ fontSize:9, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', padding:'2px 7px', borderRadius:2, background:`${p.accent}12`, border:`1px solid ${p.accent}25`, color:p.accent, marginBottom:8, alignSelf:'flex-start' }}>{p.badge}</span>
                ...
                <div style={{ fontSize:11, color:C.muted, marginBottom:8, display:'flex', alignItems:'center', gap:4 }}>
                    <BadgeCheck style={{ width:10,height:10,color:C.accent,flexShrink:0 }} />{p.seller} · {p.location}
                </div>
                <div style={{ display:'flex', alignItems:'center', gap:5, marginBottom:8 }}>
                    <div style={{ display:'flex', gap:1 }}>
                        {[1,2,3,4,5].map(i => <Star key={i} style={{ width:10,height:10,fill:i<=Math.floor(p.rating)?'#eab308':'rgba(234,179,8,0.18)',color:i<=Math.floor(p.rating)?'#eab308':'rgba(234,179,8,0.18)' }} />)}
                    </div>
                    <span style={{ fontSize:11, fontWeight:700, color:C.text }}>{p.rating}</span>
                    <span style={{ fontSize:11, color:C.dim }}>({p.reviews})</span>
                </div>
...
// ── Cart Drawer ────────────────────────────────────────────────────
function CartDrawer({ cart, onClose, onQty, onRemove }: { cart:CartItem[]; onClose:()=>void; onQty:(id:number,d:number)=>void; onRemove:(id:number)=>void }) {
        ...
        <>
            ...
            <div style={{ position:'fixed', top:0, right:0, bottom:0, width:340, background:C.bg, borderLeft:`1px solid ${C.accentBd}`, zIndex:101, display:'flex', flexDirection:'column', boxShadow:'-20px 0 60px rgba(0,0,0,0.6)' }}>
                ...
                <div style={{ flex:1, overflowY:'auto', padding:'10px 20px' }}>
                    {items.length===0
                        ? <div style={{ textAlign:'center', paddingTop:60 }}><ShoppingCart style={{ width:30,height:30,color:C.dim,margin:'0 auto 10px' }} /><div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:700, fontSize:15, textTransform:'uppercase', color:C.dim, letterSpacing:'0.06em' }}>Cart is empty</div></div>
                        : items.map(({ p, qty }) => (
                            <div key={p.id} style={{ display:'flex', gap:10, padding:'12px 0', borderBottom:`1px solid ${C.border}` }}>
                                <div style={{ width:48,height:48,borderRadius:4,flexShrink:0,background:`${p.accent}12`,border:`1px solid ${p.accent}25`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:20 }}>{p.icon}</div>
                                <div style={{ flex:1, minWidth:0 }}>
                                    ...
                                    <div style={{ fontSize:11, color:C.dim, marginBottom:8 }}>{p.seller}</div>
...
export default function BuyerPage() {
    ...
    const toggleCart = (id:number) => setCart(c => c.some(x=>x.id===id) ? c.filter(x=>x.id!==id) : [...c,{id,qty:1}]);
    const updateQty  = (id:number, d:number) => setCart(c => c.map(x => x.id===id ? {...x,qty:Math.max(1,x.qty+d)} : x));
    const removeCart = (id:number) => setCart(c => c.filter(x=>x.id!==id));
    ...
    return (
        <>
            <style>{`
                ...
                .bp-search {
                    background:rgba(0,229,255,0.04)!important; border:1px solid rgba(0,229,255,0.1)!important;
                    color:#e2e8f0!important; outline:none!important; font-family:'Barlow',sans-serif!important;
                    font-size:14px!important; transition:all 0.18s ease!important;
                }
            ...
            <div className="bp" style={{ minHeight:'100vh', background:C.bg }}>
                ...
                <div style={{ position:'sticky', top:0, zIndex:50, background:'rgba(13,27,36,0.95)', backdropFilter:'blur(20px)', borderBottom:`1px solid rgba(0,229,255,0.1)`, padding:'0 24px' }}>
                    ...
                    <div style={{ maxWidth:1200, margin:'0 auto', display:'flex', alignItems:'center', gap:16, height:56 }}>
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:9, flexShrink:0 }}>
                            ...
                            <span style={{ fontSize:18, fontWeight:700, letterSpacing:'0.01em', color:C.accent }}>
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
                            ...
                            <div style={{ position:'relative' }}>
                                <button onClick={() => setShowUserMenu(!showUserMenu)}
                                    ...
                                    {/* Avatar */}
                                    <div style={{ width:28, height:28, borderRadius:3, background:'rgba(0,229,255,0.15)', border:`1px solid ${C.accentBd}`, display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:12, color:C.accent, letterSpacing:'0.04em', flexShrink:0 }}>
                                        {initials}
                                    </div>
                                    <div style={{ textAlign:'left' }}>
                                        <div style={{ fontSize:13, fontWeight:600, color:'#cbd5e1', lineHeight:1.1, whiteSpace:'nowrap' }}>
                                            {user?.firstName ?? 'Guest'} {user?.lastName ?? 'User'}
                                        </div>
                                        <div style={{ fontSize:9, color:C.dim, fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', marginTop:1 }}>
                                            Buyer
                                        </div>
                                    </div>
                ...
                <div style={{ maxWidth:1200, margin:'0 auto', padding:'28px 24px 60px' }}>
                    ...
                    <div className="fu1" style={{ position:'relative', overflow:'hidden', borderRadius:6, marginBottom:20, padding:'26px 30px', background:C.bgCard, border:`1px solid rgba(0,229,255,0.12)` }}>
                        ...
                        <div style={{ position:'relative', maxWidth:520 }}>
                            ...
                            <div style={{ display:'flex', gap:20, flexWrap:'wrap' }}>
                                {[{v:`${PRODUCTS.length}`,l:'Products'},{v:'7',l:'Sellers'},{v:'7',l:'Categories'}].map(s => (
                    ...
                    <div className="fu4" style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
                        ...
                        <span className="sl">{activeCategory==='all'&&!query?'Featured Products':'Results'}</span>
                ...
                {cartOpen && <CartDrawer cart={cart} onClose={() => setCartOpen(false)} onQty={updateQty} onRemove={removeCart} />}
            </div>
        </>
    );
}